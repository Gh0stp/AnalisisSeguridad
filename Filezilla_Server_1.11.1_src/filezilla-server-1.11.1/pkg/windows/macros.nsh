...
!macro AsUser_Open what args
	...
	SetOutPath `$INSTDIR`
	ShellExecAsUser::ShellExecAsUser `open` `$V_AsUser_Open_What` `$V_AsUser_Open_Args` `SW_SHOWNORMAL`
!macroend
...
; Ends with, case insensitively
!macro _ends_with _a _b _t _f
	!insertmacro _LOGICLIB_TEMP
...
!macro QueryServiceStatus name
	...
	System::Call 'ADVAPI32::OpenSCManager(p 0, p 0, i 1) p.r1 ?e'
	...
	StrCmp "$1" "0" ${_Label.QueryServiceStatus}error +1 ; ${If} $1 P<> 0
		...
		StrCmp "$2" "0" ${_Label.QueryServiceStatus}error +1 ; ${If} $2 P<> 0
			Pop $0
			System::Call 'ADVAPI32::QueryServiceStatus(p r2, @ r3) i .r0 ?e'
...
!macro GrantAccess entry user properties
	...
	${If} "${user}" != ""
		...
		${If} "$GrantAccess.user_for_grant" == "LocalSystem"
			StrCpy $GrantAccess.user_for_grant "SYSTEM"
		${EndIf}

		${Log} `AccessControl::GrantOnFile "$PLUGINSDIR" "$GrantAccess.user_for_grant" ${properties}`
		AccessControl::GrantOnFile "$PLUGINSDIR" "$GrantAccess.user_for_grant" `${properties}`
		Exch $R0
		${If} $R0 == error
			Pop $R0
			Exch $R0
			MessageBox MB_OK "Unhandled error from AccessControl::GrantOnFile: $R0"
			Abort
		${EndIf}
		Pop $R0
	${EndIf}
...
!macro LastServiceLog.callback var num line
	${TrimNewLines} "${line}" "${line}"
	${Log} ${line}
	${If} "${line}" ends_with "new logging started ====="
		StrCpy "${var}" ""
	${Else}
		StrCpy "${var}" "${var}$\n${line}"
	${EndIf}
!macroend
!define LastServiceLog.callback `!insertmacro LastServiceLog.callback`

!macro CheckConfigVersionAndOwnership mode first_line rest
	${Store} CheckConfigVersionAndOwnership mode
	${MakeVar} CheckConfigDirOwnership.log ""
	...
	${GrantAccess} "$PLUGINSDIR" $CheckConfigVersionAndOwnership.user "GenericWrite + GenericRead"

	${StartService} filezilla-server '--config-version-check $CheckConfigVersionAndOwnership.mode --config-version-check-result-file "$PLUGINSDIR\config-version-check-result.txt" --config-dir-ownership-check-result-file "$PLUGINSDIR\config-dir-ownership-check-result.txt" --config-check-logger.name "$PLUGINSDIR\service-log.txt" --config-check-logger.include_headers false'
	...
	${If} ${Errors}
		${Log} "$PLUGINSDIR\service-log.txt"
		${ReadAnsiFile} "$PLUGINSDIR\service-log.txt" '${LastServiceLog.callback} "$CheckConfigDirOwnership.log"'
		${MessageBox} MB_ICONSTOP|MB_OK "$(S_CONFIG_CannotReadOwnershipResult)$\n$CheckConfigDirOwnership.log" IDOK ""
		Abort
	${EndIf}
	...
	${If} "${first_line}" != "ownership"
	    ...
	    ${OrIf} "${first_line}" == ""
			${Log} "$PLUGINSDIR\service-log.txt"
			${ReadAnsiFile} "$PLUGINSDIR\service-log.txt" '${LastServiceLog.callback} "$CheckConfigDirOwnership.log"'
		    ${MessageBox} MB_ICONSTOP|MB_OK "$(S_CONFIG_CannotReadResult)$\n$CheckConfigDirOwnership.log" IDOK ""
		    Abort
	    ${EndIf}
