...
struct version_info
{
	...
	{
		return std::forward_as_tuple(major, minor, micro, nano, rc-1, beta-1, dev) < std::forward_as_tuple(rhs.major, rhs.minor, rhs.micro, rhs.nano, rhs.rc-1, rhs.beta-1, rhs.dev);
	}
	...
	constexpr bool operator ==(const version_info &rhs) const
	{
		return std::tie(major, minor, micro, rc, nano, beta, dev) == std::tie(rhs.major, rhs.minor, rhs.micro, rhs.nano, rhs.rc, rhs.beta, rhs.dev);
	}
	...
	constexpr bool operator !=(const version_info &rhs) const
	{
		return std::tie(major, minor, micro, rc, nano, beta, dev) != std::tie(rhs.major, rhs.minor, rhs.micro, rhs.nano, rhs.rc, rhs.beta, dev);
	}
	...
	constexpr version_info(std::string_view s) noexcept
	{
		...
		if (!eol(r)) {
			...
			if (match_string(r, "-dev")) {
				if (!parse_int(r, new_info.dev))
					return;
			}
		}
	...
	friend String toString(const version_info &vi)
	{
		...
		if (vi.dev) {
			ret.append(fzS(C, "-dev")).append(toString<String>(vi.dev));
		}
...
extern const std::string full_version_string;
