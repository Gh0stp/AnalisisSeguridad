...
bool commander::buffer_operators::entry_filter::setup(std::string_view &arg)
{
	if (mode_ != disabled) {
		assert(mode_ == win && "Invalid mode for filter");

		std::string_view potential_pattern = arg;
		auto pos = arg.find_last_of('/');

		if (pos != std::string_view::npos) {
			potential_pattern.remove_prefix(pos+1);
		}

		if (potential_pattern.find_first_of("*?"sv) != std::string_view::npos) {
			arg.remove_suffix(potential_pattern.size());

		#ifdef FZ_WINDOWS
			pattern_ = fz::to_wstring_from_utf8(potential_pattern);
		#else
			pattern_ = potential_pattern;
		#endif
...
namespace {
#ifdef FZ_WINDOWS
	using wild_string_view = std::wstring_view;
	using wild_char = wchar_t;
#else
	using wild_string_view = std::string_view;
	using wild_char = char;
#endif
}

// The following function assumes
//   - that the inputs are in valid UTF8 format in non-Windows environments.
//   - that the pattern doesn't end in dots or spaces
//   - both the pattern and the string aren't empty
static constexpr bool win_wild_match(const wild_char *pat, const wild_char *pat_end, const wild_char *str, const wild_char *str_end, bool final_dot)
{
	const auto inc = [](const wild_char *&u) -> const wild_char *& {
		if constexpr (std::is_same_v<wild_char, char>) {
			...
			FZ_UTIL_BITS_ASSUME(c != 0xFFu);
			return u += util::count_leading_ones(c);
		}
		else {
			return u += 1;
		}
	...
	const auto eq = [](const wild_char &l, const wild_char &r) {
		if constexpr (std::is_same_v<wild_char, char>) {
			return l == r;
		}
		else {
			return std::towupper(l) == std::towupper(r);
		}
	};

	const wild_char *old_pat = nullptr;
	const wild_char *old_str = nullptr;

	// According to (1):
	//
	// - a '?' is a DOS_QM
	// - a '.' followed by '?' or '*' is a DOS_DOT. We keep the '?' and the '*'.
	// - a '*' followed by a '.' at the end of the pattern, is a DOS_STAR. The final '.' has been recorded in final_dot earlier.
	//
	// Then, according to (2):
	//
	// - a DOS_QM matches any single character or, upon encountering a period or end of name string, advances the expression to the end of the set of contiguous DOS_QMs.
	// - a DOS_DOT matches either a period or zero characters beyond the name string.
	// - a DOS_STAR matches zero or more characters until encountering and matching the final . in the name.
	//
	// (1) https://learn.microsoft.com/it-it/archive/blogs/jeremykuhne/wildcards-in-windows
	// (2) https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-fsa/0b034646-4e23-4874-8488-2adac231ff23
	...
	do {
		switch (*pat) {
			...
			// DOS_QM
			case '?': {
				// Matches any character, but acts specially on '.'
				// The EndOfString case is dealt generically outside the loop.
				if (*str == '.') {
					while (++pat < pat_end && *pat == '?');
				}
				else {
					++pat;
				}
			...
			case '.': {
				// If the current char in the string is a dot, then we match.
				// If it's not, we don't match, unless we've reached the EndOfString AND the next character is either a '*' or a '?' (DOS_DOT),
				// which is a case dealt with outside of the loop.
				if (*str == '.') {
					++pat;
					++str;
					continue;
		...
		str = inc(old_str);
		pat = old_pat;
	} while (pat < pat_end && str < str_end);

	/* Final fixups of pattern*/
	while (pat < pat_end && (
		*pat == '*' ||
		(*pat == '?' && str >= str_end) ||
		// This is the EndOfString case for DOS_DOT
		(*pat == '.' && (pat+1 < pat_end && (*(pat+1) == '*' || *(pat+1) == '?')) && str >= str_end)
	)) {
		++pat;
	}

	return pat >= pat_end && str >= str_end;
}

// The following functions assumes
//   - that the inputs are in valid UTF8 format
constexpr static bool win_wild_match(wild_string_view pat_view, wild_string_view str_view)
{
	// A dot at the end of the pattern means "match only files with no extension"
	auto final_dot = !pat_view.empty() && pat_view.back() == '.';

	// Remove all ending dots and spaces
	while (!pat_view.empty() && (pat_view.back() == '.' || pat_view.back() == ' ')) {
		pat_view.remove_suffix(1);
	}

	// Short-circuit corner cases
	if (pat_view.empty() != str_view.empty()) {
		return false;
	}
	...
	// The actual parsing can now begin.
	return win_wild_match(pat_view.data(), pat_view.end(), str_view.data(), str_view.end(), final_dot);
}

bool commander::buffer_operators::entry_filter::operator()(const tvfs::entry &entry) const
{
	if (pattern_.empty() || mode_ != win) {
		return true;
	}

#ifdef FZ_WINDOWS
	auto name = std::wstring_view(entry.native_name());
#else
	auto name = std::string_view(entry.name());
#endif

	if (auto pos = name.find_last_of(util::fs::native_path::separator()); pos < name.length()) {
		name = name.substr(pos+1);
	}

	return win_wild_match(pattern_, name);
}

void commander::buffer_operators::entry_filter::set_mode(mode_type mode)
{
	mode_ = mode;
}

commander::buffer_operators::entry_filter::mode_type commander::buffer_operators::entry_filter::get_mode() const
{
	return mode_;
}

bool commander::buffer_operators::entry_filter::set_mode(int mode)
{
	if (mode >= disabled && mode <= win) {
		mode_ = static_cast<mode_type>(mode);
		return true;
	}
...
commander::buffer_operators::buffer_operators(event_loop &loop, logger_interface &logger)
	: filtered_facts_lister_(loop, entries_iterator_, filter_, eol, enabled_facts_)
	, filtered_stats_lister_(loop, entries_iterator_, filter_, eol)
	, filtered_stats_lister_with_prefix_space_(loop, entries_iterator_, filter_, eol, space)
	, filtered_names_lister_(loop, entries_iterator_, filter_, eol, names_prefix_)
...
void commander::set_socket(socket_interface *si)
{
	...
	if (si) {
		...
		{
			auto res = refuse_message_.empty() ? respond<220>(true) : respond<421>(true);
...
bool commander::setup_filter(std::string_view &arg)
{
	return buffer_operators_.filter_.setup(arg);
}
...
int commander::process_buffer_line(buffer_string_view line, bool there_is_more_data_to_come)
{
	...
	if (!is_valid_utf8(cmd_line)) {
		respond<501>() << "Command or argument not in UTF8.";
		return 0;
	}
...
FTP_CMD(FEAT, none) {
	respond<211>()
		...
		<< "EPRT" << endl
		<< "MFMT" << endl
		<< "filezilla.wildcards" << buffer_operators_.filter_.get_mode() << endl
		<< "End";
}
...
FTP_CMD(OPTS, needs_arg) {
	...
	if (opts.size() == 1) {
		auto parts = fz::strtok_view(opts[0], "="sv, false);
		if (parts.size() == 2 && parts[0] == "filezilla.wildcards"sv) {
			if (!buffer_operators_.filter_.set_mode(fz::to_integral(parts[1], -1))) {
				respond<504>() << "Invalid value for filezilla.wildcards";
			}
			else {
				respond<200>() << "filezilla.wildcards set to" << parts[1];
			}
...
FTP_CMD(USER, needs_arg) {
	...
	controller_.authenticate_user(user_, authentication::just_verify{}, this);
}
...
authentication::session_user commander::handle_authenticate_user_response(std::unique_ptr<authentication::authenticator::operation> &&op)
{
	...
	if (session_user) {
		...
		if (session_user.error().is_internal()) {
...
FTP_CMD(MLSD, needs_auth | needs_data_connection)
{
	...
	return tvfs_.async_get_entries(buffer_operators_.entries_iterator_, arg, tvfs::traversal_mode::only_children, async_receive_ >> [this](auto result, const std::string &path) {
		...
		controller_.start_data_transfer(buffer_operators_.filtered_facts_lister_, this, true);
	});
}
...
FTP_CMD(LIST, needs_auth | needs_data_connection)
{
	...
	auto traversal_mode = setup_filter(arg)
		? tvfs::traversal_mode::only_children
		: tvfs::traversal_mode::autodetect;

	if (arg.empty()) {
		arg = "."sv;
	}

	return tvfs_.async_get_entries(buffer_operators_.entries_iterator_, arg, traversal_mode, async_receive_ >> [this](auto result, const std::string &path) {
		if (!result) {
			respond<550>() << strresult(result);
			return;
		}
		...
		notifier_.notify_entry_open(1, path, -1);
		controller_.start_data_transfer(buffer_operators_.filtered_stats_lister_, this, true);
	});
}
...
FTP_CMD(NLST, needs_auth | needs_data_connection)
{
	...
	return tvfs_.async_get_entries(buffer_operators_.entries_iterator_, arg, traversal_mode, async_receive_ >> [this, maybe_rel_path = util::fs::unix_path(arg)](auto result, const std::string &abs_path) {
		if (!result) {
			respond<550>() << strresult(result);
			return;
		}
		...
		controller_.start_data_transfer(buffer_operators_.filtered_names_lister_, this, true);
	});
}
...
FTP_CMD(STAT, needs_auth) {
	...
	tvfs_.async_get_entries(buffer_operators_.entries_iterator_, arg, traversal_mode, async_receive_ >> [this](result result, const std::string &path) {
		if (!result) {
			respond<550>() << strresult(result);
			return;
		}
		...
		process_nested_adder_until_eof(buffer_operators_.filtered_stats_lister_with_prefix_space_, [this](int err) {
