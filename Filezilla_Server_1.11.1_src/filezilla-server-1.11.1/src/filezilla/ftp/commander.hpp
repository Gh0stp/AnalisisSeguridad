...
public:
	...
	{
		struct entry_filter: buffer_operator::tvfs_entry_filter
		{
			enum mode_type
			{
				disabled,
			...
			bool setup(std::string_view &arg);
			bool operator()(const tvfs::entry &entry) const override;
		...
		entries_lister<tvfs::entry_facts, tvfs::entry_facts::which&> filtered_facts_lister_;
		entries_lister<tvfs::entry_stats> filtered_stats_lister_;
		entries_lister<buffer_operator::with_prefix<tvfs::entry_stats>, std::string_view> filtered_stats_lister_with_prefix_space_;
		entries_lister<buffer_operator::with_prefix<tvfs::entry_name>, std::string&> filtered_names_lister_;
...
private:
	...
	bool setup_filter(std::string_view &arg);
