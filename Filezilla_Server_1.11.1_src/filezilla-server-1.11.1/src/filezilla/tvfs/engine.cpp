...
result engine::remove_entry(const entry &entry, bool recursive)
{
	result res = { result::other, FZ_RESULT_RAW(ERROR_TIMEOUT, ETIMEDOUT) };

	async_remove_entry(entry, recursive, timeout_receive_ >> std::tie(res, std::ignore));
...
void engine::async_open_file(file_holder &out_file, std::string_view tvfs_path, file::mode mode, int64_t rest, receiver_handle<completion_event> r)
{
	...
	file::creation_flags flags;
	if (!rest) {
		flags = file::empty;
	}
	else if (rest == rest_mode::fresh) {
		flags = file::fresh;
	}
	else {
		flags = file::existing;
	...
	return backend_->open_file(resolved_path.native_path, mode, flags, async_receive(r)
...
void engine::async_remove_entry(entry entry, bool recursive, receiver_handle<completion_event> r)
{
	...
	if (entry.type_ == entry_type::dir) {
		return backend_->remove_directory(entry.native_name_, recursive, async_receive(r)
		>> [r = std::move(r), path = std::move(entry.name_)] (auto res) {
			r(res, std::move(path));
		});
	}
...
void engine::async_set_current_directory(std::string_view tvfs_path, receiver_handle<simple_completion_event> r)
{
	resolve_path(tvfs_path).async_to_entry(backend_, async_receive(r) >> [this, r = std::move(r)](result res, entry &e) mutable {
		...
		if (!(e.perms_ & (permissions::apply_recursively|permissions::read|permissions::list_mounts))) {
...
resolved_path engine::resolve_path(std::string_view path)
{
	...
	auto elements = canonical_path.elements_iterable();

	auto [node, ei, native_path] = mount_tree_->resolve_path(elements);
	auto perms = node.perms;

	if (!(perms & permissions::apply_recursively) && ei != elements.end() && std::next(ei) != elements.end())
		perms = {};
