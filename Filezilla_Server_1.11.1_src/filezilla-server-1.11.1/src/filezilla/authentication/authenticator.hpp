...
public:
	...
	virtual void authenticate(std::string_view realm, std::string_view user_name, const methods_list &methods, address_type family, std::string_view ip, event_handler &target, logger::modularized::meta_map meta_for_logging = {}) = 0;
...
{
	void authenticate(std::string_view, std::string_view, const methods_list &, address_type, std::string_view, event_handler &, logger::modularized::meta_map meta_for_logging = {}) override;

	void stop_ongoing_authentications(event_handler &) override;

	void add_realm(std::string);
	void remove_realm(std::string);
};
