...
bool token_manager::verify(std::string_view username, const refresh_token &token, impersonation_token &impersonation [[maybe_unused]])
{
	...
	if (our.must_impersonate) {
		impersonation_token imp(fz::to_native(username), impersonation_options::pwless, logger_);
		if (!imp) {
...
refresh_token token_manager::create(const shared_user &su, duration expiration, std::string_view path)
{
	...
	if (auto u = su->lock()) {
		...
		needs_impersonation = u->impersonation_token && fz::to_utf8(u->impersonation_token.username()) == u->name;
		name = u->name;
