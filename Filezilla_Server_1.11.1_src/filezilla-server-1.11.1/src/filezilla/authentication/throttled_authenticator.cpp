...
public:
	worker(throttled_authenticator &owner, std::string_view realm, std::string_view name, address_type family, std::string_view ip, event_handler *target, logger::modularized::meta_map meta_for_logging)
...
bool throttled_authenticator::worker::authenticate()
{
	if (next_op_) {
		...
		owner_.wrapped_.authenticate(realm_, name_, methods_, family_, ip_, *this, std::move(meta_for_logging_));
	}
...
void throttled_authenticator::authenticate(std::string_view realm, std::string_view name, const methods_list &methods, address_type family, std::string_view ip, fz::event_handler &target, logger::modularized::meta_map meta_for_logging)
{
