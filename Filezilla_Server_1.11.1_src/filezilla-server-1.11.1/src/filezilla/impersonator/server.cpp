...
int fz::impersonator::server::run()
{
	...
	while (!error_ && !(error_ = channel_.recv(any))) {
		...
		bool dispatched = fz::rmp::dispatch<
			...
			fz::impersonator::messages::set_credentials
		>(std::move(any), this,
			&server::open_file,
...
void fz::impersonator::server::set_credentials(const tvfs::backend::credentials_map &creds)
{
	backend_.set_credentials(creds, sync_receive >> [&](auto &res) {
		error_ = channel_.send(fz::impersonator::messages::set_credentials_response(res));
	});
}
