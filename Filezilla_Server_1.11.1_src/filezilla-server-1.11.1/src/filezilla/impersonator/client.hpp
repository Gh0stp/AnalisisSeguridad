...
{
public:
	client(thread_pool &thread_pool, logger_interface &logger, const impersonation_token &token = get_null_impersonation_token(), native_string_view exe = {}, std::size_t pool_size = 1);
	~client() override;
	...
	void set_credentials(const tvfs::backends::local_filesys::credentials_map &creds, receiver_handle<set_credentials_response> r) override;
...
private:
	...
	void init(caller &c, receiver_handle<> r);
	...
	const impersonation_token &token_{};
	native_string exe_;
	std::size_t pool_size_{};

	std::optional<messages::set_credentials> set_credentials_message_;

	callers callers_available_;
	callers callers_in_use_;

	static const impersonation_token &get_null_impersonation_token();
};
