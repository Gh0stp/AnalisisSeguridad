...
public:
	...
	client::callers::iterator operator->()
	{
		return *caller_.first;
	}

	client::callers::value_type &operator*()
	{
		return **caller_.first;
	}
	...
	{
		return bool(caller_.first);
	}

	bool is_new() const
	{
		return caller_.second;
	}
	...
	~get_caller()
	{
		if (caller_.first) {
			...
			client_.callers_available_.splice(client_.callers_available_.end(), client_.callers_in_use_, *caller_.first);
			client_.condition_.signal(lock);
		}
	}

private:
	std::pair<std::optional<client::callers::iterator>, bool> get()
	{
		fz::scoped_lock lock(client_.mutex_);
		...
		return {caller, is_new_caller};
	}

	client &client_;
	std::pair<std::optional<client::callers::iterator>, bool> caller_;
};

client::client(thread_pool &thread_pool, logger_interface &logger, const impersonation_token &token, native_string_view exe, std::size_t pool_size)
	: thread_pool_(thread_pool)
	, event_loop_(thread_pool_)
	, logger_(logger, "impersonator client", { { "filesys user", token ? fz::to_utf8(token.username()) : "<invalid tocken>"} })
	, token_(token)
	, exe_(exe)
...
auto client::call(receiver_handle<E> &&r, Args &&... args) -> decltype(std::declval<caller>().call(T(std::forward<Args>(args)...), std::move(r)))
{
	if (auto c = get_caller(*this)) {
		if constexpr (!std::is_same_v<T, messages::set_credentials>) {
			if (c.is_new() && set_credentials_message_) {
				logger_.log_raw(logmsg::debug_info, L"sending stored credentials to the new caller before invoking the requested method.");
				c->call<tvfs::backend::set_credentials_response>(*set_credentials_message_, async_receive(r) >> [](auto){});
			}

			c->call(T(std::forward<decltype(args)>(args)...), std::move(r));
		}
		else {
			set_credentials_message_.emplace(args...);
			c->call(*set_credentials_message_, std::move(r));
		}
	}
	else {
		std::apply(r, messages::default_for<rmp::make_message_t<E>>()(result{result::other}).tuple());
	}
}
...
void client::set_credentials(const tvfs::backends::local_filesys::credentials_map &creds, receiver_handle<set_credentials_response> r)
{
	call<messages::set_credentials>(std::move(r), creds);
}

const impersonation_token &client::get_null_impersonation_token()
{
	static const impersonation_token token;
	return token;
}
