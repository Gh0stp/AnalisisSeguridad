...
public:
	...
	void set_value(const value_info<T> &vi) {
		...
		if (vi.info) {
			if (cur_node_.last_child() == info_node) {
				cur_node_.remove_child(info_node);
			}
			else {
...
public:
	...
	struct options: util::options<options, xml_input_archive> {
		...
		using postload_func_type = std::function<error_t (native_string_view filename, pugi::xml_node root, build_info::version_info &product_version)>;
		...
		opt <postload_func_type> postload_func        = o(nullptr);
		options(){}
	};
