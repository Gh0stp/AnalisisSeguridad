	...
	, root_node_name_{opts_.root_node_name().c_str()}
{
	...
	auto act_on_version_check_result = [&](auto error_str) {
		if (error_str.empty()) {
			return;
		}
		...
		if (opts_.verify_version() == options::verify_mode::backup) {
			...
			if (auto res = util::io::copy(docpath, backup, fz::file::current_user_and_admins_only); !res) {
				error({EIO, fz::sprintf("Failed making backup copy of %s. Reason: %s.", docpath, strresult(res))});
				return;
			}
	...
	act_on_version_check_result(error_str);

	if (!error_) {
		if (opts_.postload_func()) {
			if (auto err = opts_.postload_func()(fz::util::fs::native_path(loader.name()).base(), cur_node_, product_version_)) {
				error(std::move(err));
			}
		}
