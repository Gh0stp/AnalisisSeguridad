...
namespace fz::serialization::detail
{
	...
	struct archive_error_with_suberror {
		...
		archive_error_with_suberror &operator=(archive_error_with_suberror &&rhs) noexcept {
			archive_error_with_suberror tmp(std::move(rhs));
		...
		friend void swap(archive_error_with_suberror &lhs, archive_error_with_suberror &rhs) noexcept
		{
			using std::swap;

			swap(lhs.suberror_, rhs.suberror_);
