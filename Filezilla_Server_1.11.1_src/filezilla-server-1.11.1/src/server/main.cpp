...
namespace {
	...
	int config_checks_result(std::string_view config_check, const fz::native_string &result_file, bool success, const std::vector<fz::native_string> &backups_made, fz::logger_interface &logger)
	{
		...
		if (!result_file.empty()) {
			...
			if (auto res = file.open(result_file, fz::file::writing, fz::file::empty | fz::file::current_user_and_admins_only); !res) {
				logger.log_u(fz::logmsg::error, L"Could not open file '%s' for writing. Reason: %s.", result_file, strresult(res));
				return EXIT_FAILURE;
			}

			auto res = [&]() {
				auto res = fz::util::io::write(file, result);
				if (!res) {
					return res;
				}

				res = fz::util::io::write(file, '\n');
				if (!res) {
					return res;
				}
				...
				if (result == "backup") {
					for (const auto &b: backups_made) {
						res = fz::util::io::write(file, fz::to_string(b).append("\n"));
						if (!res) {
							return res;
						}
					}
			...
			if (!res) {
				logger.log_u(fz::logmsg::error, L"Could not write config-check results to file '%s'. Reason: %s.", result_file, strresult(res));
				return EXIT_FAILURE;
			}
		}
...
int FZ_SERVICE_PROGRAM_MAIN(argc, argv) {
	...
	auto start_server = [&](int argc, char *argv[]) -> int {
		...
		argv_input_archive{argc, argv}(
			...
			optional_nvp{file_logger_options, "config-check-logger"},
			optional_nvp{print_version, "version"}
		);

		if (print_version) {
			std::puts(fz::build_info::full_version_string.c_str());
			return EXIT_SUCCESS;
		}
		...
		file_logger.set_options(std::move(file_logger_options));
		...
		if (config_dir_was_empty && is_system && !config_already_existed) {
			auto restore = [&](const fz::util::fs::native_path &old) {
				...
				if (auto res = fz::util::io::copy_dir(old, config_paths.for_writing(), fz::mkdir_permissions::cur_user_and_admins)) {
					...
					logger.log_u(fz::logmsg::status, L"Something went wrong: not all files were copied. The server might malfunction. Error: %s", fz::strresult(res));
				}
			};
		...
		fz::native_string impersonator_exe;
			...
			fz::util::fs::native_path_list webui_app_root_list;
				...
				if (auto p = fz::util::get_abs_top_src_directory(FZ_REL_TOP_SRCDIR)) {
					webui_app_root_list = p / fzT("src")/fzT("filezilla")/fzT("webui")/fzT("app")/fzT("dist")/fzT("spa");
				}
			#endif

			if (webui_app_root_list.empty()) {
				#ifdef FZ_UNIX
					webui_app_root_list = (fz::known_paths::data::dirs()/fzT("filezilla-server")/fzT("webui"));
				#else
					webui_app_root_list = (fz::util::get_own_executable_directory()/fzT("webui"));
				#endif
			}
		...
		{
			xml_upgrader upgrader(logger);

			argv_input_archive ar{argc, argv, {
				{ config_paths.settings(fz::file::reading), true, xml_input_archive::options().postload_func(std::ref(upgrader.settings)) },
			...
			ar(
				...
				value_info{optional_nvp{file_logger_options, "config-check-logger"}, "Logger settings that apply only for the startup phase"}
				#ifdef ENABLE_FZ_WEBUI
