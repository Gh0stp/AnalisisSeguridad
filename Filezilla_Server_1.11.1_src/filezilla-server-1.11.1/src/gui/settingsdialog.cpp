...
bool SettingsDialog::Create(wxWindow *parent, const wxString &server_name, fz::util::fs::path_format server_path_format, wxWindowID winid, const wxPoint &pos, const wxSize &size, long style, const wxString &name)
{
	...
	static_cast<wxTreebook *>(GetBookCtrl()) | [&](auto p) {
		...
		authentication_page_ = wxPage(p, _S("Rights management")) | [&](auto p) {
			wxVBox(p, 0) = wxCreate<wxNotebook>(p, nullID) |[&](auto *p) {
				wxPage(p, _S("Default system user for filesystem access")) | [&](auto *p) {
					...
					wxVBox(p) = {
						...
						radio_other = new wxRadioButton(p, 1, _S("Use a &different system user")),
						panel_other = new wxPanel(p) | [&] (auto p) {
							wxVBox(p) =	default_impersonator_book_ = wxCreate<wxNavigationEnabled<wxSimplebook>>(p) | [&](auto p) {
								...
								wxPage(wxValidateOnlyIfCurrent)(p, wxS("*NIX*")) |[&](auto p) {
									wxVBox(p, 0) = {
										{ 0, wxLabel(p, _S("N&ame:")) },
										{ 0, default_impersonator_name_nix_ = new wxTextCtrl(p, wxID_ANY) },

										{ 0, wxLabel(p, _S("G&roup (optional, leave blank to pick the default one):")) },
				...
				wxPage(p, _S("Protocol policies")) |[&](auto *p) {
					wxVBox(p) = realms_editor_ = wxCreate<RealmsEditor>(p, RealmsEditor::for_system);
				};

				if (server_path_format_ == fz::util::fs::windows_format) {
					wxPage(p, _S("Windows shares credentials")) |[&](auto *p) {
						wxVBox(p) = mount_credentials_editor_ = wxCreate<MountCredentialsEditor>(p);
					};
				}
			};
...
void SettingsDialog::SetGroupsAndUsers(const fz::authentication::file_based_authenticator::groups &groups, const fz::authentication::file_based_authenticator::users &users, bool can_impersonate, const fz::native_string server_username)
{
	...
	if (mount_credentials_editor_) {
		mount_credentials_editor_->SetCredentials(users_.mount_credentials);
	}

	groups_editor_->SetServerPathFormat(server_path_format_);
	users_editor_->SetServerPathFormat(server_path_format_);

	realms_editor_->SetRealms(&users_.realms);
...
void SettingsDialog::SetWebUIOptions(const fz::webui::server::options &webui_options, const fz::securable_socket::cert_info::extra &tls_extra_info)
{
	...
	webui_refresh_token_timeout_ctrl_->SetRef(webui_options_.refresh_token_timeout, fz::duration::from_days(0), fz::duration::from_days(90))
	->set_mapping({
		{fz::duration::from_seconds(0), _S("0 (Disabled) - Users will be logged out when the access token expires")}
	});
}
...
void SettingsDialog::SetGenerateSelfsignedCertificateFunction(CertInfoEditor::GenerateSelfsignedFunc func)
{
	...
	admin_cert_info_ctrl_->SetGenerateSelfsignedCertificateFunction(func);
...
void SettingsDialog::OnGroupsListChanging(GroupsList::Event &ev)
{
	...
	auto ask_user_what_to_do_on_deletion = [&] () -> state_e {
		...
		choices.reserve(groups_.size()+2);
		choices.push_back(_F("Unassign the group \"%s\" from users", ev.GetOldName()));
		choices.push_back(_F("Delete users who belong only to the group \"%s\"", ev.GetOldName()));
		choices.push_back(_F("Delete all users of the group \"%s\"", ev.GetOldName()));

		for (const auto &g: alternative_groups) {
			choices.push_back(_F("Reassign users to group \"%s\"", fz::to_wxString(g)));
		}
		...
		int idx = wxGetSingleChoiceIndex(
			_F("Some users still belong to the group \"%s\".\n\n"
			   "Select an update option for the affected users:", ev.GetOldName()),
		...
		new_name = alternative_groups[std::size_t(idx-3)];
...
#ifdef ENABLE_FZ_WEBUI
	if (webui_tls_extra_info_.expired()) {
		protocols_with_expired_certs.push_back(_S("WebUI"));
	}
#endif
