...
#include "../server/xml_upgrader.hpp"
...
struct ServerAdministrator::config_parts
{
	...
	std::optional<wxString> load(fz::native_string_view src, fz::logger_interface &logger);
...
private:
	...
	loading_error load_current(fz::serialization::xml_input_archive::file_loader &loader,  fz::serialization::xml_input_archive::options::verify_mode verify_mode, fz::logger_interface &logger);
...
std::optional<wxString> ServerAdministrator::config_parts::load(fz::native_string_view src, fz::logger_interface &logger)
{
	using namespace fz::serialization;

	xml_input_archive::file_loader loader{fz::native_string(src)};
	auto error = load_current(loader, xml_input_archive::options::verify_mode::error, logger);
	...
	if (auto mismatch = error.flavour_or_version_mismatch()) {
		...
		if (res == wxID_YES) {
			error = load_current(loader, xml_input_archive::options::verify_mode::ignore, logger);
		}
		else {
			return {};
		}
...
ServerAdministrator::config_parts::loading_error ServerAdministrator::config_parts::load_current(fz::serialization::xml_input_archive::file_loader &loader, fz::serialization::xml_input_archive::options::verify_mode verify_mode, fz::logger_interface &logger)
{
	using namespace fz::serialization;
	...
	xml_input_archive ar(loader, xml_input_archive::options()
		 ...
		 .postload_func(std::ref(upgrader.imported_configuration_file))
	);
