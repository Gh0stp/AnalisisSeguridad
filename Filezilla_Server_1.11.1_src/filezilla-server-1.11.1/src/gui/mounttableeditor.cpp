...
class MountTableEditor::AccessChoice: public wxChoice
{
public:
	using access = fz::tvfs::mount_point::access_t;

	constexpr static const access accesses[] = {
		access::disabled,
		access::read_only,
		access::write_only,
		access::read_write,
	...
	AccessChoice(wxWindow *parent)
		: wxChoice(parent, nullID)
	{
		Append(_S("Disabled"));
		Append(_S("Read only"));
		Append(_S("Write only"));
		Append(_S("Read + Write"));
	...
	{
		auto it = std::find(std::begin(accesses), std::end(accesses), a);
		auto sel = std::distance(std::begin(accesses), it);

		wxASSERT_MSG(sel >= 0 && sel < int(std::size(accesses)), "Access type not recognized");
	...
	access GetAccess() const
	{
		...
		wxASSERT_MSG(sel >= 0 && sel < int(std::size(accesses)), "Invalid selection");

		return accesses[sel];
	}

private:
	using wxChoice::SetSelection;
	using wxChoice::GetSelection;
};
...
bool MountTableEditor::Create(wxWindow* parent, wxWindowID winid, const wxPoint& pos, const wxSize& size, long style, const wxString& name)
{
	...
	wxGBox(this, 2, {0}, {0}, wxGBoxDefaultGap, wxALIGN_TOP) = {
		...
		{ wxSizerFlags(0).Expand(), wxStaticVBox(this, _S("Mount options")) = perms_ = wxCreate<wxNavigationEnabled<wxSimplebook>>(this) | [&](wxSimplebook *p) {
			wxPage(p, wxS("**enabled**")) | [&](auto p) {
				wxVBox(p, 0) = {
					wxLabel(p, _S("Access mod&e:")),
					access_ = new AccessChoice(p),
					recursive_ = new wxCheckBox(p, wxID_ANY, _S("Apply permissions to su&bdirectories")),
					modify_structure_ = new wxCheckBox(p, wxID_ANY, _S("Wri&table directory structure")),
					autocreate_ = new wxCheckBox(p, wxID_ANY, _S("&Create native directory if it does not exist"))
				};
			};
	...
	auto on_access_change = [this, on_recursive_change, on_modify_structure_change](fz::tvfs::mount_point::access_t v, fz::tvfs::mount_point *mp) {
		...
		switch (v) {
			...
			case fz::tvfs::mount_point::write_only:
	...
	auto select_row = [this, on_access_change, on_recursive_change, on_modify_structure_change](int row) {
		if (auto mp = table_->GetMountPoint(row)) {
			...
			on_access_change(access_->GetAccess(), mp);
		}
	};
	...
	access_->Bind(wxEVT_CHOICE, [on_access_change, this](wxCommandEvent &ev){
		...
		on_access_change(access_->GetAccess(), grid_->GetCurrentMountPoint());
	});
