...
class tvfs_test final : public CppUnit::TestFixture
{
	...
	CPPUNIT_TEST(test_write_only_root);
	CPPUNIT_TEST(test_cwd);
	CPPUNIT_TEST(test_cwd_with_virtual_root);
	CPPUNIT_TEST(test_non_recursive_root);
	CPPUNIT_TEST(test_holes);
	CPPUNIT_TEST(test_remove<fz::tvfs::mount_point::read_write>);
	CPPUNIT_TEST(test_rename<fz::tvfs::mount_point::read_write>);
	CPPUNIT_TEST(test_set_mtime<fz::tvfs::mount_point::read_write>);
	CPPUNIT_TEST(test_remove<fz::tvfs::mount_point::write_only>);
	CPPUNIT_TEST(test_rename<fz::tvfs::mount_point::write_only>);
	CPPUNIT_TEST(test_set_mtime<fz::tvfs::mount_point::write_only>);
...
public:
	...
	void test_write_only_root();
	void test_cwd();
	void test_cwd_with_virtual_root();
	void test_non_recursive_root();
	void test_holes();
	template <fz::tvfs::mount_point::access_t access>
	void test_remove();
	template <fz::tvfs::mount_point::access_t access>
	void test_rename();
	template <fz::tvfs::mount_point::access_t access>
...
void tvfs_test::test_cwd()
{
	...
	CPPUNIT_ASSERT_EQUAL(fz::result::nofile, res.error_);
...
void tvfs_test::test_cwd_with_virtual_root()
{
	set_mount_table({
		{ "/actual_root", native_root_, fz::tvfs::mount_point::read_write, fz::tvfs::mount_point::do_not_apply_permissions_recursively }
	});

	auto res = tvfs_.set_current_directory(tvfs_root_);
	CPPUNIT_ASSERT_EQUAL(fz::result::ok, res.error_);

	res = tvfs_.set_current_directory(tvfs_root_ / "actual_root");
	CPPUNIT_ASSERT_EQUAL(fz::result::ok, res.error_);
...
void tvfs_test::test_write_only_root()
{
	{
		auto f = (native_root_ / native_test_file(0)).open(fz::file::writing, fz::file::creation_flags::empty);
		CPPUNIT_ASSERT(f.opened());
	...
	set_mount_table({
		{ "/", native_root_, fz::tvfs::mount_point::write_only, fz::tvfs::mount_point::do_not_apply_permissions_recursively }
	});
...
void tvfs_test::test_holes()
{
	...
	// It should not be able to list /foo/bar
	CPPUNIT_ASSERT_EQUAL(std::string("bar"), it.next().name());
	CPPUNIT_ASSERT(!it.has_next());
}
...
void tvfs_test::test_remove()
{
	...
	set_mount_table({
		{ "/dir0", native_root_, access, fz::tvfs::mount_point::apply_permissions_recursively_and_allow_structure_modification },
	});
	...
	res = tvfs_.remove_entry(it.next(), false);
	CPPUNIT_ASSERT_EQUAL(fz::result::noperm, res.error_);
	...
	if (access == fz::tvfs::mount_point::read_write) {
		res = tvfs_.get_entries(it, tvfs_root_ / "dir0" / test_dir(0), fz::tvfs::traversal_mode::only_children);
	}
	else {
		res = tvfs_.get_entries(it, tvfs_root_ / "dir0" / test_dir(0) / test_file(0), fz::tvfs::traversal_mode::no_children);
	}
	...
	if (access == fz::tvfs::mount_point::read_write) {
		...
		CPPUNIT_ASSERT_EQUAL(std::string(tvfs_root_ / "dir0" / test_dir(0) / test_file(0)), file_entry.name());
	}

	res = tvfs_.remove_entry(file_entry, false);
	CPPUNIT_ASSERT_EQUAL(fz::result::ok, res.error_);
...
void tvfs_test::test_rename()
{
	...
	res = fz::file().open(native_root_ / native_test_dir(1) / native_test_file(0), fz::file::writing);
	CPPUNIT_ASSERT_EQUAL(fz::result::ok, res.error_);

	set_mount_table({
		{ "/dir0", native_root_ / native_test_dir(0), access, fz::tvfs::mount_point::apply_permissions_recursively_and_allow_structure_modification },
		{ "/dir1", native_root_ / native_test_dir(1), access, fz::tvfs::mount_point::apply_permissions_recursively_and_allow_structure_modification }
	});
...
void tvfs_test::test_set_mtime()
{
	...
	set_mount_table({
		{ "/", native_root_, access, fz::tvfs::mount_point::apply_permissions_recursively_and_allow_structure_modification },
	});
